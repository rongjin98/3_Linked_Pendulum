...
[time, z] = ode23(@(t,y) manipulatorInverseDynamics(t,y,B_f,N_f), tspan, z_0, options);
...
for i = 1:length(time)
    ...
    %[x,v,a] = SinusoidalSwingUp(0.55,0.5,time(i));
    x = 0.5;
    v = 0;
    a = 0;
    Xd(:,i) = [x(1) 1.25]';
...
subplot(2,1,1)
plot(time,Xd(1,:)-Xe(1,:));
title("Endeffector Error Step Response (0.5m)")
xlabel("Time (sec)");
ylabel("X Position Error (m)");
yline(xsetpoint*0.05,'--');
yline(xsetpoint*-0.05,'--');
xlim([0,bound])
ylim([-0.1,0.5])
...
xlabel('X Position (m)');
ylabel('Y Position (m)');
title("Step Response in X and Y Partial Compensation")
